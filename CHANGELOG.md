...
- align verify-publish allowlist with renamed logo files
...
- **Centered hero on the docs landing pages.** Removed the `image:` block from `docs/index.md` and `docs/tr/index.md` hero frontmatter (so the banner no longer appears next to the title text on `/codewikitap-mcp/` and `/codewikitap-mcp/tr/`), and added matching CSS in `docs/.vitepress/theme/style.css` (`.VPHero .container`, `.main`, `.actions`) so the name, text, tagline, and action buttons are horizontally centered with a sane max-width.
- **Author attribution on the docs landing pages.** Small centered footer line at the bottom of `docs/index.md` ("Developed by Burak Arslan") and `docs/tr/index.md` ("Burak Arslan tarafından geliştirilmiştir"), each linking to the author's GitHub profile.
