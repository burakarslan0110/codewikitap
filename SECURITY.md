...
- **MCP protocol surface.** CodeWiKiTap registers 7 read-only tools plus 1 non-read-only tool (`request_indexing`). The tool surface is locked at runtime (regex `/(search|ask|query|generate|index|write)/i` with a single-name whitelist for `request_indexing`); plugins or hooks cannot extend it. See [`.claude/rules/codewiki-mcp-mcp-protocol.md`](.claude/rules/codewiki-mcp-mcp-protocol.md) for the locked-surface rationale.
